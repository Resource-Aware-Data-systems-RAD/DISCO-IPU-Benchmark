...
constexpr char kModelPath[] =
    "apps/cubesat_benchmark_memcpy/model_edgetpu_025.tflite";
constexpr char kImagePath[] = "/apps/cubesat_benchmark_memcpy/cat_224x224.rgb";
...
void Main() {
  ...
  uint16_t tot_height = 1824;
	uint16_t tot_width = 1824; //1824 if 100%
		...
		for ( uint16_t height_offset = 0; height_offset < tot_height; height_offset += tile_size ) {
			...
			for ( uint16_t width_offset = 0; width_offset < tot_width; width_offset += tile_size ) {
				...
				for (uint16_t h = height_offset; h < tile_size + height_offset; h++ ) {
					memcpy(input + ((h-height_offset) * tile_size * tot_channels), large_img + (h * tot_width * tot_channels + width_offset * tot_channels), sizeof(uint8_t) * tile_size * tot_channels);
				}
